...
def visualize_errors_chau(error_tuples, relation_map) -> Tuple[List, List]:
    """
    ...
    for sent, token, isAgreeError, isWordOrderError, isAssignmentError, agreement_rules_not_followed, wordorder_rules_not_followed, assignment_rules_not_followed in error_tuples:
        ...
        if isAgreeError:
            agreement_examples_per_rules = findWordsWhereAgreementNotFollowed(agreement_rules_not_followed, sent, sent_tokens, token, relation_map)

        if isWordOrderError:
            wordorder_examples_per_rules = findWordsWhereWordOrderNotFollowed(wordorder_rules_not_followed, sent,
                                                                       sent_tokens, token, relation_map)
        if isAssignmentError:
            casemarking_examples_per_rules = findWordsWhereMarkingNotFollowed(assignment_rules_not_followed, sent,
                                                                              sent_tokens, token, relation_map)
...
def visualize_conll_errors_chau(error_tuples, relation_map):
    ...
    for sent, token, isAgreeError, isWordOrderError, isAssignmentError, agreement_rules_not_followed, wordorder_rules_not_followed, assignment_rules_not_followed in error_tuples:
        ...
        conll_str += [f'<h1> The tokens of interest (i.e. have errors according to our rules) are marked in ***, hover over the ***-marked tokens for more grammar information </h1>']

        if isAgreeError:
            agreement_examples_per_rules = findWordsWhereAgreementNotFollowed(agreement_rules_not_followed, sent,
                                                                              sent_tokens, token, relation_map)
            erro_feats = list(agreement_examples_per_rules.keys())
            conll_str += [f'<h3> Error in morphological agreement for <b> {", ".join(erro_feats)} </b> i.e. the ***-marked tokens should have matching gender values </h3>']
            conll_str += [f"<div class='bibtex' id='{idx}'>"]
        ...
        if isWordOrderError:
            ...
            conll_str += [f'<h3> Error in word order for <b> {", ".join(erro_feats)} </b> i.e. the token and its syntactic head are not in the correct order </h3>']
            conll_str += [f"<div class='bibtex' id='{idx}'>"]
            conll_str += [get_conll_str(sent, token.id)]
            conll_str += ["</div>"]
        ...
        if isAssignmentError:
            ...
            conll_str += [f'<h3> Error in case marking for <b> {", ".join(erro_feats)} </b> i.e. the case value for *** marked token is not correct </h3>']
            conll_str += [f"<div class='bibtex' id='{idx}'>"]
            conll_str += [get_conll_str(sent, token.id)]
            conll_str += ["</div>"]
...
def findWordsWhereAgreementNotFollowed(rules_not_followed, sent, sent_tokens, token, relation_map):
    ...
    for model, info in rules_not_followed.items():
        ...
        for (one_active, one_nonactive, label) in info:
            ...
            active_text, nonactive_text = getActiveFeatures(one_active, one_nonactive, "agreement", model, relation_map)
            rules_per_features[model] = (sent_example_tokens, active_text, nonactive_text, token_feature_value, headtoken_feature_value)
...
def findWordsWhereWordOrderNotFollowed(rules_not_followed, sent, sent_tokens, token, relation_map):
    ...
    for model, info in rules_not_followed.items():
        ...
        # )
        #Readable active features
        active_text, nonactive_text = getActiveFeatures(one_active, one_nonactive, "wordorder", model, relation_map)
...
def findWordsWhereMarkingNotFollowed(rules_not_followed, sent, sent_tokens, token, relation_map):
    ...
    for model, info in rules_not_followed.items():
        ...
        for (one_active, one_nonactive, label) in info:
            ...
            # )
            #Readable active features
            active_text, nonactive_text = getActiveFeatures(one_active, one_nonactive, "assignment", model, relation_map)
