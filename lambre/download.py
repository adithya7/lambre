...
def download_lambre_files(lg: str, resource_mapping: Dict, parser_dir: Path, rules_dir: Path):
    ...
    subprocess.run(["wget", "-q", "--show-progress", resource_mapping[lg]["rules_pratapa_etal_2021"], "-O", local_path / f"{lg}.txt"])
    # download rules for chaudhury-etal-2021
    local_path = rules_dir / "chaudhury-etal-2021"
    local_path.mkdir(exist_ok=True, parents=True)
    subprocess.run(["wget", "-q", "--show-progress", resource_mapping[lg]["rules_chaudhury_etal_2021"], "-O", local_path / f"{lg}.txt"])
    ...
    subprocess.run(["wget", "-q", "--show-progress", resource_mapping["resources.json"], "-O", parser_dir / "resources.json"])

    # download parser and decompress
    subprocess.run(["wget", "-q", "--show-progress", resource_mapping[lg]["parser"], "-O", parser_dir / f"{lg}.tar.gz"])
